...
export const ProjectSchema = z.object({
  slug: z.string().min(1, 'Slug là bắt buộc').trim(),
  ...
  responsibilities: z.array(z.string()).min(1, 'Phải có ít nhất một trách nhiệm'),
  ...
  color: z.string().regex(/^bg-/, 'Màu sắc phải bắt đầu bằng class Tailwind (bg-...)'),
  icon: z.string().min(1, 'Icon là bắt buộc'),
  link: z.string().url({ message: 'Đường dẫn không hợp lệ' }).nullable().optional()
});
