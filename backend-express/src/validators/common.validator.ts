...
export const ExperienceSchema = z.object({
  ...
  responsibilities: z.array(z.string()).min(1, 'Phải có ít nhất một nhiệm vụ'),
...
export const SkillSchema = z.object({
  ...
  level: z.string().min(1, 'Mức độ là bắt buộc'),
...
export const ProfileSchema = z.object({
  ...
  socials: z.record(z.string(), z.url({ message: 'Đường dẫn mạng xã hội không hợp lệ' })).optional()
});
